...
pub fn Header() -> impl IntoView {
    ...
    view! {
        <header class="flex sticky top-0 w-full bg-transparent z-800">
            ...
            <A href=format!("{BASE_URL}/") {..} class="flex gap-2 m-r-auto nav-btn h-10 p-1 group">
                    {config
                        .avatar
                        .clone()
                        .map(|avatar| {
                ...
                <A href=format!("{BASE_URL}/posts") {..} class="h-10 gap-1 nav-btn text-sm p-x-4">
                    "文章"
                </A>
                <A href=format!("{BASE_URL}/notes") {..} class="h-10 gap-1 nav-btn text-sm p-x-4">
                    "笔记"
                </A>
                // <A href=format!("{BASE_URL}/search") {..} class="h-10 gap-1 nav-btn text-sm p-x-4">
                //     "搜索"
                // </A>
                {config
                    .github_owner
